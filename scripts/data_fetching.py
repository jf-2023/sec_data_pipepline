...
def fetch_all_files(directory):
    ...
    for file in tqdm(files[:1700], desc='Synchronous Progress'):
...
async def fetch_all_files_async(directory):
    """Opens all files in a selected directory asynchronously"""
    files = os.listdir(directory)
    tasks = []
    for file in tqdm(files[:1700], desc='Asynchronous Progress'):
        full_path = os.path.join(directory, file)
...
def fetch_all_files_threading(directory):
    """Opens all files in a selected directory using threading"""
    files = os.listdir(directory)
    with ThreadPoolExecutor() as executor:
        list(tqdm(executor.map(fetch_file, [os.path.join(directory, file) for file in files[:1700]]),
                 total=1700, desc='Threading Progress'))


def fetch_all_files_multiprocessing(directory):
    """Opens all files in a selected directory using multiprocessing"""
    files = os.listdir(directory)
    with ProcessPoolExecutor() as executor:
        list(tqdm(executor.map(fetch_file, [os.path.join(directory, file) for file in files[:1700]]),
                 total=1700, desc='Multiprocessing Progress'))
...
def main():
    ...
    fetch_all_files(company_data_dir)
    ...
    start_time = time.perf_counter()
    asyncio.run(fetch_all_files_async(company_data_dir))
    ...
    start_time = time.perf_counter()
    fetch_all_files_threading(company_data_dir)
    ...
    start_time = time.perf_counter()
    fetch_all_files_multiprocessing(company_data_dir)
